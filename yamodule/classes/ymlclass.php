...
class yml
{
	...
	{
		$allowed = array('url', 'price', 'currencyId', 'categoryId', 'picture', 'store', 'pickup', 'delivery', 'vendor', 'model', 'description', 'sales_notes', 'downloadable', 'weight', 'dimensions', 'param', 'sales_notes', 'country_of_origin');
		$param = array();
		$param = $data['param'];
		foreach($data as $k => $v)
		{
		...
		$this->offers[] = array('id' => $id, 'data' => $data, 'available' => ($available) ? 'true' : 'false', 'type' => 'vendor.model');
	}
